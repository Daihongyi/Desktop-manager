...
def clean():
    mj=easygui.buttonbox(msg="确定清除结果吗？", choices=["确定", "取消"], title="Desktop Manager 5.0")
    if mj == "确定":
        a = os.listdir("D:\\Desktop Manager运行根目录\\")
        if a == []:
            easygui.msgbox("没有可以清除的文件", title="Desktop Manager 5.0")
        else:
            for i in a:
...
def xuijishu():
    global l
    a = easygui.multenterbox(msg="请输入以下项", fields=["请输入随机数最小值", "请输入随机数最大值", "请输入生成数量"], title="Desktop Manager 5.0")
    ...
    for i in a:
        ...
        if nv != x:
            b = b + ","
            nv= nv+1
    easygui.msgbox(msg="结果为" + b, title="Desktop Manager 5.0")
    rtime = localtime()
...
def dakai():
    global l
    g = easygui.enterbox("请输入文件路径", title="Desktop Manager 5.0")
    os.system(g)
    l += 1
...
def zhishupanduan():
    ...
    global l
    a = easygui.enterbox("请输入", title="Desktop Manager 5.0")
    a = int(a)
    b = 0
    c = []
    if a == 0 or a == 1:
        easygui.msgbox("本数不为质数或合数", title="Desktop Manager 5.0")
    else:
        ...
        if b == 2:
            easygui.msgbox("质数", title="Desktop Manager 5.0")
            easygui.msgbox("它的约数有" + str(c), title="Desktop Manager 5.0")
            rtime = localtime()
            time = strftime("%Y-%m-%d %H:%M:%S",rtime)
            ...
            rtime = localtime()
            time = strftime("%Y-%m-%d %H:%M:%S", rtime)
            easygui.msgbox("合数", title="Desktop Manager 5.0")
            easygui.msgbox("它的约数有" + str(c), title="Desktop Manager 5.0")
            with open('D:\\Desktop Manager运行根目录\\' + time + "质数判断结果.txt", "w") as f:
...
def jiamijiemi():
    global l
    a = easygui.buttonbox(msg="你需要加密还是解密", choices=["加密", "解密"], title="Desktop Manager 5.0")
    if a == "加密":
        b = easygui.enterbox("请输入要加密的内容", title="Desktop4.6")
        c = easygui.enterbox("请输入要加密的格数（输入数字即可，最大为9，最小为1）", title="Desktop Manager 5.0")
        c = int(c)
        d = ""
        for i in b:
            e = ord(i)
        ...
        easygui.msgbox("加密结果为" + c + d, title="Desktop Manager 5.0")
        rtime = localtime()
        time = strftime("%Y-%m-%d %H:%M:%S", rtime)
    ...
    if a == "解密":
        b = easygui.enterbox("请输入要解密的内容", title="Desktop Manager 5.0")
        ...
        for i in range(len(b) - 1):
            ...
            d = d + e
            d = str(d)
        easygui.msgbox("解密结果为" + d, "Desktop Manager 5.0")
        rtime = localtime()
        time = strftime("%Y-%m-%d %H:%M:%S", rtime)
        with open('D:\\Desktop Manager运行根目录\\' + time + r"解密结果.txt", "w", encoding="gbk") as f:
            f.write("解密结果为" + d)
        l += 1
bn=""
def paoyingbi():
    global l,bn
    a = easygui.enterbox("输入抛硬币的次数", title="Desktop Manager 5.0")
    ...
    for i in range(a):
        ...
        if c == 1:
            f = f + 1
            bn = bn + "本次结果为反\r\n"
    easygui.msgbox("正面有" + str(z) + "次" + "," + "反面有" + str(f) + "次", title="Desktop Manager 5.0")
    rtime = localtime()
    time = strftime("%Y-%m-%d %H:%M:%S", rtime)
...
def wenjianjiashengcheng():
    global l
    a = easygui.enterbox("请输入需要生成文件夹的目录", title="Desktop Manager 5.0")
    b = easygui.enterbox("请输入需要生成文件夹的名称", title="Desktop Manager 5.0")
    os.chdir(a)
    os.makedirs(b)
...
def main():
    while True:
        ...
        easygui.msgbox("欢迎", title="Desktop Manager 5.0")
